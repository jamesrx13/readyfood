...
class _WorkerRegisterPageState extends State<WorkerRegisterPage> {
  ...
  var idRestaurantController = TextEditingController();

  void registerNewWorker(BuildContext context) async {
    // Espera
    showDialog(
    ...
    try {
      ...
      if (userData.user != null) {
        DatabaseReference newUser = FirebaseDatabase.instance
            .ref()
            .child('worker/${userData.user?.uid}');
        String token = await PushNotificationServices.getCellphoneToken();
        Map userDataMap = {
          'fullName': fullNameController.text.trim(),
          'email': emailController.text.trim(),
          'phone': phoneController.text.trim(),
          'restaurantAssoc': idRestaurantController.text.trim(),
          'phoneToken': token,
        };
        ...
        Navigator.pop(context);
        AllToast.showDangerousToast(context, 'Oh, Oh...', 'Usuario NO creado');
      }
    ...
    return OsudButton(
      ...
      onPressed: () async {
        ...
        if (phoneController.text.length < 10) {
          AllToast.showInfoToast(context, 'Oh, Oh...', 'Télefono no válido');
        ...
        if (idRestaurantController.text.length < 6) {
          AllToast.showInfoToast(context, 'Oh, Oh...', 'Falta su restaurante');

          return;
        }
        registerNewWorker(context);
      },
    );
  }
  ...
  Widget _campoRestaurant() {
    InputDecoration decoration = InputDecoration(
      ...
      labelText: 'Restaurante asociado',
    ...
    return GestureDetector(
      onTap: () async {
        ...
        if (barcode == null) {
          ...
          idRestaurantController.text = barcode;
        }
      },
      child: TextField(
